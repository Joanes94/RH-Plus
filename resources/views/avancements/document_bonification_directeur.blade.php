...
<head>
    <meta charset="UTF-8">
    <title>Accord de bonification (centre) — {{ $personnel->nom_complet }}</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pj { margin-top: 20px; font-size: .88rem; }
        .pj .titre-pj { text-decoration: underline; font-weight: 700; }
...
<div class="no-print">
    ...
    <a class="btn-alt" href="{{ route('avancements.document', $avancement) }}">Voir la lettre à l'employé</a>
...
<div class="doc-page">
    ...
    <div class="dest">
        Au<br>
        <span class="qui">Révérend Père Directeur du {{ $centre->nom ?? $contrat->centre ?? 'Centre' }}</span><br>
        {{ strtoupper($ville) }}
    </div>

    <div class="ref">N/RÉF : {{ $avancement->numero_reference }}</div>
    <div class="objet">
        Objet : Accord de bonification<br>
        à {{ $civilite }} {{ $personnel->prenoms }} {{ strtoupper($personnel->nom) }}
    </div>

    <div class="doc-body" style="margin-top: 20px;">
        <p>Révérend Père Directeur,</p>
        <p>Conformément à l'Accord d'Établissement applicable aux personnels des Institutions Sanitaires Diocésaines de Cotonou du 11 Décembre 2019, en son article 88, {{ $civilite }} {{ $personnel->prenoms }} {{ strtoupper($personnel->nom) }}, bénéficie d'un coefficient de <strong>{{ number_format((float) $avancement->coefficient_applique, 1) }}</strong> du salaire de sa catégorie à 24 mois de sa retraite.</p>
        <p>Donc à 24 mois de sa retraite, son salaire de base passe de <strong>{{ $fmtArgent($avancement->ancien_salaire) }} FCFA</strong>, à <strong>{{ $fmtArgent($avancement->nouveau_salaire) }} FCFA</strong>.</p>
        <p>Vous trouverez ci-joint, une lettre lui notifiant cet accord.</p>
        <p>Veuillez recevoir, Révérend Père Directeur, nos meilleures salutations.</p>
    </div>

    <div class="pj">
        <div class="titre-pj">Pièce jointe :</div>
        <div>Accord de bonification 01</div>
    </div>
    ...
    <div class="signature" style="margin-top: 30px;">
        ...
        @if($avancement->statut === 'valide')
            <div class="sig-image-wrap" style="text-align: right; margin-top: 10px;">
                @if(!empty($signature_url ?? $signature_directeur_url))
                    <img src="{{ $signature_url ?? $signature_directeur_url }}" alt="Signature DDIS" style="height: 120px; width: auto; object-fit: contain;">
                @endif
            </div>
